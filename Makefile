...
android_includes = $(android_source) $(android_source)/system/core/include $(android_source)/system/media/camera/include $(android_source)/frameworks/native/include $(android_source)/frameworks/av/include $(android_source)/frameworks/av/services/camera/libcameraservice $(android_source)/hardware/libhardware/include
...
android_libs = binder cameraservice gui hardware utils
...
$(soccervision_objects): $(out)/%.o : %.cpp
	@mkdir -p $(dir $@)
	./android-compiler-clang compile $(debug_flags) $(include_flags) $(android_include_flags) -std=c++11 -MMD -c $< -o $@
