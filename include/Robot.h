...
public:
  ...
  const Math::Position getPosition() const {
    return Math::Position(x, y, orientation);
  }
  ...
  float getTravelledRotation() {
    return travelledRotation;
  }
  bool isStalled();
  bool hasTasks() {
  ...
  Odometer::Movement getMovement() {
    return movement;
  }
  const ParticleFilterLocalizer::Measurements &getMeasurements() const {
    return measurements;  // TODO Here?
  }
